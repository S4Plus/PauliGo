...
from functions import go_synthesis1
...
def tree_synthesis1(qc, graph, pauli_map, ptree, psd, parameter): # psd : pauli string
    ps = psd.ps
    psn = ps2nodes(ps)
    ...
    while lfs != []:
        ...
        if l.depth == 0:
            ...
            qc.rz(parameter, l.pid) # qc.rz(2*psd.real+1, l.pid)
            break
...
def block_opt_SC(pauli_layers, pauli_map=None, graph=None, qc=None, arch='manhattan', synthesis_opt = False):
    ...
    for i1 in pauli_layers: # i1 : a layer
        for i2 in i1[:1]: # i2 : a block
            ...
            #         print('e2')
            #         input()
            parameter = Parameter('phi' + str(parameter_index))
            for i3 in i2:
                if synthesis_opt:
